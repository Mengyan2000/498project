...
class Trainer:
    ...
    def __setup__(self, trainer_configs):
        ...
        # if self.trainer_configs.resume_from_best or self.trainer_configs.eval_model: # because eval_model = True, and we don't want to load new model if quantization happens
        if self.trainer_configs.resume_from_best or self.trainer_configs.eval_model:
            self.load_ckpt()
            self.trainer_configs.model = quantization.quantize_model(self.trainer_configs.model, trainer_configs)
    ...
    def update_saved_model(self, new_benchmarking_accuracy, new_benchmarking_loss, new_train_accuracy, new_train_loss):
        ...
        if self.metric_func(new_metric, self.curr_metric):
            ...
            print("reload ckpt in update_saved_model")
    ...
    def eval(self, data_loader, print_acc=True, cfm=False):
        ...
        if cfm:
            plotter.plot_confusion_matrix(confusion_matrix.numpy().astype(int), normalize=True, save_path = 'C:/Users/mengy/Documents/University/ECE498/Final_project/images/confusion_matrix', nclass=nclass)
